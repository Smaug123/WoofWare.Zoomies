...
module NumKeyBits =
    ...
    let toInt (i : NumKeyBits) : int = i
    let pow2 (i : NumKeyBits) : int64 = 1 <<< i
    let add (a : NumKeyBits) (b : NumKeyBits) : NumKeyBits = a + b

// the log2 of a number of nanoseconds.
type AlarmPrecision = AlarmPrecision of int
module AlarmPrecision =
  let numKeyBits (AlarmPrecision t) = t |> NumKeyBits.ofInt
  let intervalNum (AlarmPrecision t) (time : TimeNs) : int64 =
      (time |> TimeNs.to_int63_ns_since_epoch) >>> t

  let toSpan (AlarmPrecision t) =
      if t < 0 then failwith "toSpan of negative power of two nanoseconds"
      (1 <<< t)
  ...
  let mul (AlarmPrecision t) (AlarmPrecision pow2) = t + pow2 |> AlarmPrecision
  let div (AlarmPrecision t) (AlarmPrecision pow2) = t - pow2 |> AlarmPrecision

  let intervalNumStart (AlarmPrecision t) (intervalNum : int64) : TimeNs =
    intervalNum <<< t
    |> TimeNs.of_int63_ns_since_epoch

  let ofSpanFloorPow2Ns span =
    if span <= TimeNs.Span.zero then failwith "got non-positive span"
    span |> Time_ns.Span.to_int63_ns |> Int64.floor_log2
...
module TimingWheel =
    ...
    module LevelBits =
          ...
          let numBitsInternal t = List.fold NumKeyBits.add NumKeyBits.zero t
          let numBits t : int = numBitsInternal t

          let createThrowing (extendToMaxNumBits : bool option) ints =
            if List.isEmpty ints then failwith "LevelBits.createThrowing requires a nonempty list"
            if List.exists (fun bits -> bits <= 0) ints then failwith "LevelBits.createThrowing got nonpositive num bits"
            let numBits = List.sum ints
            if numBits > maxNumBits then failwith "LevelBits.createThrowing got too many bits"
            let extendToMaxNumBits = extendToMaxNumBits |> Option.defaultValue false
            let ints =
              if extendToMaxNumBits
              then ints @ List.replicate 1 (maxNumBits - numBits)
              else ints
            List.map NumKeyBits.ofInt ints

          let default' = createThrowing None [ 11; 10; 10; 10; 10; 10; 1 ]

          let trim (t : LevelBits) maxNumBits : LevelBits =
            if numBitsInternal t <= maxNumBits then t
            else
              let rec loop t remaining =
                ...
                | b :: t ->
                  if  b >= remaining
                  then [ remaining ]
                  else b :: loop t (remaining - b)
              loop t maxNumBits

(* [{max,min}_time] are bounds on the times supported by a timing wheel. *)

    let maxTime = TimeNs.maxValueRepresentable
    ...
    module Config =
      let levelBitsDefault = LevelBits.default'
      let alarmPrecision (t : Config) = AlarmPrecision.toSpan t.AlarmPrecision
      ...
      let maxNumLevelBits alarmPrecision =
          NumKeyBits.maxValue - (AlarmPrecision.numKeyBits alarmPrecision)

      let create (capacity : int option) (levelBits : LevelBits option) (alarmPrecision : AlarmPrecision) () =
        let levelBits = levelBits |> Option.defaultValue levelBitsDefault
      ...
      let microsecondPrecision () =
          create None (LevelBits.createThrowing None [10;10;6;6;5] |> Some) AlarmPrecision.aboutOneMicrosecond

      let durations (t : Config) : System.TimeSpan list =
        List.mapFold (fun numBitsAcc levelNumBits ->
            let numBitsAcc = numBitsAcc + (levelNumBits |> NumKeyBits.toInt) in
            let duration =
              TimeNs.Span.of_int63_ns
                (if numBitsAcc = 64 - 1
                 then System.Int64.MaxValue
                 else 1L <<< numBitsAcc)
            numBitsAcc, duration
        )
            (AlarmPrecision.numKeyBits t.AlarmPrecision |> NumKeyBits.toInt)
            t.LevelBits
        |> fst
