...
/// Internal events generated by the framework for visual feedback and other framework-managed behaviors.
type internal FrameworkEvent = | ActivationVisualTimeout of NodeKey
...
type WorldFreezer<'appEvent> =
    private
        {
            ...
            /// Internal framework events for visual feedback timeouts, etc.
    ...
    /// Post an internal framework event (e.g., for visual feedback timeouts).
    member internal this.PostInternalEvent (evt : FrameworkEvent) = this._InternalEvents.Enqueue evt

    /// Drain all pending internal framework events.
    member internal this.DrainInternalEvents () : FrameworkEvent seq =
        seq {
            let mutable evt = Unchecked.defaultof<_>

            while this._InternalEvents.TryDequeue &evt do
                yield evt
        }
