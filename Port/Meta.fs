...
module private MetaTypeIds =
    ...
    // Nothing.t equivalent - using FakeUnit as placeholder
    let nothingTypeId = TypeId.create<FakeUnit> "Nothing.t"
...
module Model =
    ...
    and ModelId<'a> =
        | Leaf of TypeId<'a>
        | Tuple of TupleModelIdCrate
        | Map of MapModelIdCrate  
        | MapOn of MapOnModelIdCrate
        | MultiModel of MultiModelIdCrate
    
    and TupleModelIdEval<'ret> =
        abstract Eval<'a1, 'a2> : 'a1 ModelId * 'a2 ModelId -> 'ret
    
    and TupleModelIdCrate =
        abstract Apply<'ret> : TupleModelIdEval<'ret> -> 'ret
    
    and MapModelIdEval<'ret> =
        abstract Eval<'k, 'result, 'cmp> : TypeId<'k> * TypeId<'cmp> * 'result ModelId -> 'ret when 'k : comparison
    
    and MapModelIdCrate =
        abstract Apply<'ret> : MapModelIdEval<'ret> -> 'ret
    
    and MapOnModelIdEval<'ret> =
        abstract Eval<'k_model, 'k_io, 'result, 'cmp_model> : 
            TypeId<'k_model> * TypeId<'k_io> * TypeId<'cmp_model> * 'result ModelId -> 'ret when 'k_model : comparison
    
    and MapOnModelIdCrate =
        abstract Apply<'ret> : MapOnModelIdEval<'ret> -> 'ret
    ...
    module TupleModelIdCrate =
        let make<'a1, 'a2> (a : 'a1 ModelId) (b : 'a2 ModelId) : TupleModelIdCrate =
            { new TupleModelIdCrate with
                member _.Apply eval = eval.Eval<'a1, 'a2> (a, b) }
    
    [<RequireQualifiedAccess>]
    module MapModelIdCrate =
        let make<'k, 'result, 'cmp when 'k : comparison> (k : TypeId<'k>) (cmp : TypeId<'cmp>) (by : 'result ModelId) : MapModelIdCrate =
            { new MapModelIdCrate with
                member _.Apply eval = eval.Eval<'k, 'result, 'cmp> (k, cmp, by) }
    
    [<RequireQualifiedAccess>]
    module MapOnModelIdCrate =
        let make<'k_model, 'k_io, 'result, 'cmp_model when 'k_model : comparison> 
            (kModel : TypeId<'k_model>) 
            (kIo : TypeId<'k_io>) 
            (cmp : TypeId<'cmp_model>) 
            (by : 'result ModelId) : MapOnModelIdCrate =
            { new MapOnModelIdCrate with
                member _.Apply eval = eval.Eval<'k_model, 'k_io, 'result, 'cmp_model> (kModel, kIo, cmp, by) }
    ...
    module HiddenModel =
        ...
        let sexpOf (hidden : HiddenModel) : string =
            hidden.Apply { new HiddenModelEval<string> with
                member _.Eval (model, info) = info.SexpOf model }
        
        let equal (hidden1 : HiddenModel) (hidden2 : HiddenModel) : bool =
            hidden1.Apply { new HiddenModelEval<bool> with
                member _.Eval (m1, info1) =
                    hidden2.Apply { new HiddenModelEval<bool> with
    ...
    let map<'k, 'cmp, 'a when 'k : comparison> 
        (kTypeId : TypeId<'k>) 
        (cmpTypeId : TypeId<'cmp>) 
        (model : 'a Model) : Map<'k, 'a> Model = {
        Default = Map.empty
        Equal = fun m1 m2 -> 
            if Map.count m1 <> Map.count m2 then false
            else Map.forall (fun k v1 -> 
                match Map.tryFind k m2 with 
                | Some v2 -> model.Equal v1 v2 
                | None -> false) m1
        TypeId = Map (MapModelIdCrate.make kTypeId cmpTypeId model.TypeId)
        SexpOf = fun m -> 
            let pairs = Map.toSeq m |> Seq.map (fun (k, v) -> sprintf "%A: %s" k (model.SexpOf v))
            sprintf "Map[%s]" (String.concat "; " pairs)
    }
    
    let ofModule<'a when 'a : equality> (sexpOf : 'a -> string) (equal : ('a -> 'a -> bool) option) (defaultValue : 'a) (name : string) : 'a Model =
...
module MultiModel =
    ...
    let sexpOf (mm : MultiModel) : string =
        let pairs = Map.toSeq mm |> Seq.map (fun (k, v) -> sprintf "%d: %s" k (Model.HiddenModel.sexpOf v))
        sprintf "MultiModel[%s]" (String.concat "; " pairs)
    
    let findExn (mm : MultiModel) (key : int) : Model.HiddenModel =
        Map.find key mm
    
    let set (mm : MultiModel) (key : int) (data : Model.HiddenModel) : MultiModel =
        Map.add key data mm
    ...
    let modelInfo (defaultValue : MultiModel) : MultiModel Model.Model = {
        ...
        Equal = fun m1 m2 -> 
            if Map.count m1 <> Map.count m2 then false
                ...
                | None -> false) m1
        TypeId = Model.MultiModel (Model.MultiModelIdCrate.make defaultValue)
        SexpOf = sexpOf
    }
...
module MetaInput =
    ...
    let map<'k, 'cmp, 'a when 'k : comparison> (k : TypeId<'k>) (cmp : TypeId<'cmp>) (by : Input<'a>) : Input<Map<'k, 'a>> =
        Model.Map (Model.MapModelIdCrate.make k cmp by)
    
    let create<'a> () : Input<'a> =
        Model.Leaf (TypeId.create<'a> "input")
        ...
        module InputHidden =
            let make<'input, 'key> (input : 'input) (typeId : Input<'input>) (key : 'key) : InputHidden<'key> =
                { new InputHidden<'key> with
                    member _.Apply eval = eval.Eval<'input, 'key> (input, typeId, key) }
