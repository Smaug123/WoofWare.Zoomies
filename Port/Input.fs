...
module Input =
    ...
    let map (t : Input<'a>) (f : 'a -> 'b) : Input<'b> =
        ...
        | Join joinCrate ->
            joinCrate.Apply
                { new InputJoinEval<'a, Input<'b>> with
                    member _.Eval (input, g) =
                        Join (InputJoinCrate.make input (fun x -> f (g x)))
                }
    ...
    let iterIncremental (t : Input<'a>) (f : NodeCrate -> unit) : unit =
        ...
        | Join joinCrate ->
            joinCrate.Apply
                { new InputJoinEval<'a, FakeUnit> with
                    member _.Eval (incr, _) =
                        f (NodeCrate.make incr)
                        FakeUnit
                }
            |> ignore<FakeUnit>
    ...
    let toIncremental (input : Input<'a>) : 'a Node =
        ...
        | Join joinCrate ->
            joinCrate.Apply
                { new InputJoinEval<'a, 'a Node> with
    ...
    let merge (a : Input<'a>) (b : Input<'b>) : Input<'a * 'b> =
        ...
        | Dynamic a, Join joinCrateB ->
            joinCrateB.Apply
                { new InputJoinEval<'b, Input<'a * 'b>> with
                    member _.Eval (b, f) =
                        Join (InputJoinCrate.make (I.Both a b) (fun (a, b) -> a, f b))
                }
        | Static a, Join joinCrateB ->
            joinCrateB.Apply
                { new InputJoinEval<'b, Input<'a * 'b>> with
                    member _.Eval (b, f) =
                        Join (InputJoinCrate.make b (fun b -> a, f b))
                }
        | Join joinCrateA, Static b ->
            joinCrateA.Apply
                { new InputJoinEval<'a, Input<'a * 'b>> with
                    member _.Eval (a, f) =
                        Join (InputJoinCrate.make a (fun a -> f a, b))
                }
        | Join joinCrateA, Dynamic b ->
            joinCrateA.Apply
                { new InputJoinEval<'a, Input<'a * 'b>> with
                    member _.Eval (a, f) =
                        Join (InputJoinCrate.make (I.Both a b) (fun (a, b) -> f a, b))
                }
                ...
                { new InputJoinEval<'a, Input<'a * 'b>> with
                    member _.Eval (a, f) =
                        joinCrateB.Apply
                            { new InputJoinEval<'b, Input<'a * 'b>> with
                                member _.Eval (b, g) =
                                    Join (InputJoinCrate.make (I.Both a b) (fun (a, b) -> f a, g b))
                            }
                }
