...
and Leaf1Eval<'ret> =
    abstract Eval<'model, 'dynamic_action, 'input when 'model : equality> :
        Leaf1Data<'model, 'dynamic_action, 'input> -> 'ret

and Leaf1Crate =
    abstract Apply<'ret> : Leaf1Eval<'ret> -> 'ret

and Leaf1Data<'model, 'dynamic_action, 'input when 'model : equality> =
    {
        Model : Model<'model>
        InputId : MetaInput<'input>
        DynamicAction : 'dynamic_action TypeId
        ApplyAction :
            ('dynamic_action -> Effect.Effect<unit>)
...
and Leaf0Eval<'ret> =
    abstract Eval<'model, 'static_action when 'model : equality> : Leaf0Data<'model, 'static_action> -> 'ret

and Leaf0Crate =
    abstract Apply<'ret> : Leaf0Eval<'ret> -> 'ret

and Leaf0Data<'model, 'static_action when 'model : equality> =
    {
        Model : Model<'model>
        StaticAction : 'static_action TypeId
        ApplyAction :
            ('static_action -> Effect.Effect<unit>)
                -> (Effect.Effect<unit> -> unit)
                -> 'model
                -> 'static_action
                -> 'model
...
and LeafIncrData<'input, 'result> =
    {
        Input : Value<'input>
        Compute :
            WoofWare.Incremental.Incremental -> WoofWare.Incremental.Node<'input> -> WoofWare.Incremental.Node<'result>
    }
...
module Computation =
    ...
    let return' (value : Value<'result>) : Computation<'result> = Return value

    /// Create a lazy computation
    let lazy' (computation : Lazy<Computation<'result>>) : Computation<'result> = Lazy' computation
    ...
    let switch
        (matchValue : Value<int>)
        (arms : Map<int, Computation<'result>>)
        (here : string)
        : Computation<'result>
        =
        Switch
            { new ComputationSwitchCrate<'result> with
                member _.Apply eval =
                    eval.Eval
                        {
                            Match = matchValue
    ...
    let leaf1<'model, 'dynamic_action, 'input when 'model : equality>
        (model : Model<'model>)
        (inputId : MetaInput<'input>)
        (dynamicAction : 'dynamic_action TypeId)
        (applyAction :
        ...
        (reset : ('dynamic_action -> Effect.Effect<unit>) -> (Effect.Effect<unit> -> unit) -> 'model -> 'model)
        (input : Value<'input>)
        : Computation<'model * ('dynamic_action -> Effect.Effect<unit>)>
        =
        Leaf1
            { new Leaf1Crate with
                member _.Apply eval =
    ...
    let leaf0<'model, 'static_action when 'model : equality>
        (model : Model<'model>)
        (staticAction : 'static_action TypeId)
        (applyAction :
            ('static_action -> Effect.Effect<unit>)
                ...
                -> 'model)
        (reset : ('static_action -> Effect.Effect<unit>) -> (Effect.Effect<unit> -> unit) -> 'model -> 'model)
        : Computation<'model * ('static_action -> Effect.Effect<unit>)>
        =
        Leaf0
            { new Leaf0Crate with
                member _.Apply eval =
